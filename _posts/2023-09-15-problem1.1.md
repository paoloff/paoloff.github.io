...
It can be easily seen that the function $F_j(x)$ consists of a symmetric triangular peak centered at $x=\frac{j+1/2}{n}$, with a base consisting of the segment $\Delta x = [\frac{j-1/2}{n},\frac{j+3/2}{n}] $ and height $y = f(\frac{j+1/2}{n})$. 
